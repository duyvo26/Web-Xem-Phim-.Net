...
namespace WebPhimV1.Code.ashx
{
    ...
    public class GuiBaoCaoLoi : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (context.Request.QueryString["mail_user"] != null && context.Request.QueryString["noidung_user"] != null)
            {
                mail_user = context.Request.QueryString["mail_user"]; // id user bao cao loi
                ...
                urls = "/cp-admin/tap-phim/capnhat-" + id_Tap;
                    var dt = (from q in dl.DB_USERs where Convert.ToInt32(q.quyen_han) != 0 select q).ToList();
                    foreach (var txt in dt)
                    {
                        NguoiDung.ThemThongBao(txt.id_user, Convert.ToInt32(mail_user), send_noidung, urls);
                    }
